...
class InvoiceController extends Controller
{
    public function store(Request $request){
        ...
        if ($productCode == null || $productCode->product_code == null) {$productCode = Item::where('name',$request->name)->first();}
        $data2=new Order;
        $data2->email= $request->email;
        $data2->product_code = $productCode->product_code ? $productCode->product_code : $productCode->item_code;
        ...
        if($products == null){$products = DB::table('items')->where('category',$request->item)->first();}
    ...
    public function formData($id){
        ...
        if($product == null) {
           $product = Item::where('item_code',$order->product_code)->first();
        }
        //dd($product);
        if ($product  == null) { $product = Item::where('item_code',$order->product_code)->first(); }
